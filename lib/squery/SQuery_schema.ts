...
export const SQuery_Schema = (
  description: DescriptionSchema,
  options?:
    | SchemaOptions<FlatRecord<any>, {}, {}, {}, {}>
    | ResolveSchemaOptions<{}>
  ...
  schema.pre("save", async function () {
    ...
    this.__updatedProperty = (this.modifiedPaths() as string[]).filter(
      (updatedProperty) => {
        const rule = description[updatedProperty];
        return Array.isArray(rule)
          ? rule[0].access !== "secret"
          : rule?.access !== "secret";
      }
    );
  });
